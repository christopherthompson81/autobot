...
class Movements {
  ...
  getMoveDiagonal (node, dir, neighbors) {
    ...
    // Blocks two up matter
    const blockE = this.getBlock(node, dir.x, 2, dir.z)
    const blockE1 = this.getBlock(node, 0, 2, dir.z)
    const blockE2 = this.getBlock(node, dir.x, 2, 0)

    // 100 as the threshold may be a lot
    ...
    if (cost > 100) return
    cost += this.safeOrBreak(blockE1, toBreak)
    if (cost > 100) return
    cost += this.safeOrBreak(blockE2, toBreak)
    if (cost > 100) return
    ...
    cost += this.safeOrBreak(blockB, toBreak)
    if (cost > 100) return
    cost += this.safeOrBreak(blockE, toBreak)
    if (cost > 100) return
